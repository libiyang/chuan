...
namespace YahooHit
{
    public partial class Form1 : Form
    {
        ...
        private string mainUrl = "http://tw.search.yahoo.com/";
        private void ClearCookie()
        {
            Controls.Remove(this.wbYahoo);
            Controls.Remove(this.wbBlog);
            Controls.Remove(this.wbBlogDetail);
        ...
        private void refreshTimer_OnRefresh(string keyWord, string strLinks)
        {
            try
            {
                ...
                HtmlElement kwInput = this.wbYahoo.Document.GetElementById("yschsp");
                if (kwInput == null)
                {
                    this.Navigate(this.wbYahoo, this.mainUrl);
                    kwInput = this.wbYahoo.Document.GetElementById("yschsp");
                }
                HtmlElement goBtn = this.wbYahoo.Document.GetElementById("yschbt");
                ...
                for (int iPage = 1; iPage < 20; iPage++)
                {
                    ...
                    else
                    {
                        if (iPage > 1)
                        {
                            string purl = "http://tw.search.yahoo.com/search?p=" + HttpUtility.UrlEncode(keyWord) + "&fr=yfp&ei=utf-8&v=0&b=" + (10 * (iPage - 1) + 1);
                    ...
                    try
                    {
                        ...
                        {
                            string ttt = wbYahoo.Document.Links[j].OuterHtml;
                            if (ttt.IndexOf("yschttl", StringComparison.CurrentCultureIgnoreCase) == -1)
                            {
                                continue;
                            }
                            foreach (string link in links)
                            {
                                ...
                                if (url.IndexOf(link, StringComparison.CurrentCultureIgnoreCase) > -1 && url.IndexOf("login.yahoo.com", StringComparison.CurrentCultureIgnoreCase) == -1 && url.IndexOf("knowledge.yahoo.com/question", StringComparison.CurrentCultureIgnoreCase) == -1 && url.IndexOf("cache", StringComparison.CurrentCultureIgnoreCase) == -1 && url.IndexOf("search.yahoo.com", StringComparison.CurrentCultureIgnoreCase) == -1)
                                {
                                    //點擊搜索結果
                                    ...
                                    {
                                        if ((lnkBlog.OuterHtml.IndexOf("article") > -1 || lnkBlog.OuterHtml.IndexOf("/post/") > -1 || lnkBlog.OuterHtml.IndexOf(".html") > -1) && (lnkBlog.InnerText != null && lnkBlog.InnerText.Length > 12))
                                        {
                                            if (link.IndexOf("http://mypaper.pchome.com.tw/") > -1)
                                            {
                                                if (lnkBlog.OuterHtml.IndexOf(link.Replace("http://mypaper.pchome.com.tw/", "")) == -1)
                                                {
                                                    continue;
                                                }
                                            }
                                            if (link.IndexOf("http://blog.yam.com/") > -1)
                                            {
                                                if (lnkBlog.OuterHtml.IndexOf(link.Replace("/user/", "/").Replace(".html", "")) == -1)
                                                {
                                                    continue;
                                                }
                                            }
